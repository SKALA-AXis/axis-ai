...
# 작성자: 최종민
# 변경이력:
#   2026-05-21 최종민 — Layer B 분석·카드뉴스·벡터 인덱싱 파이프라인 초기 작성
#   2026-06-02 박지원 — 분석 파이프라인 에이전트 재구성 및 카드 검증·뉴스 통합 개선
"""Layer B 분석·카드뉴스·벡터 인덱싱 파이프라인.

develop 의 PreprocessingService (Layer A) 이후 실행된다.
AnalysisGraphRunner → CardNewsComposer 흐름은 AnalysisPipelineRunner 가 담당하며,
