...
# 작성자: 최종민
# 변경이력:
#   2026-05-21 최종민 — Layer B 분석 파이프라인(분석 플로우 그래프) 최초 구축, 이후 Qdrant 선례 검색 어댑터 주입 및 as_of point-in-time 백필 배선
#   2026-05-22 심유정 — 에이전트 아키텍처·믹서 인사이트 정련, 전략 인사이트 에이전트 구현 및 카드뉴스 근거화 개선
#   2026-05-26 박지원 — 분석 파이프라인 에이전트 재구성·명칭 정리, 레거시 에이전트 제거 및 카드뉴스 표시 데이터 동기화
