...
# 작성자: 박지원
# 변경이력:
#   2026-05-08 박지원 — 글로벌 peer 기업 BCG 크롤러 추가 및 소스 포매팅, 이후 preprocess 반영
#   2026-05-26 최종민 — langfuse tracing 누락 LLM 호출 패치
...
6. 제목/description/URL 기준으로 모니터링 sector 관련 글만 저장
...
def match_bcg_core_sectors(
    *,
    ...
    """BCG 글의 핵심 메타 기준 sector 매칭.

    본문 전체에 sector 단어가 한두 번 스치는 글까지 수집하면 BCG publications의
    금융/소비재/패션 일반 리포트가 많이 섞인다. 수집 단계에서는 제목, 요약,
    URL slug처럼 글의 중심 주제가 드러나는 필드만 사용한다.
    """
    haystack = f"{title} {description} {url}"
    sectors = [sector for sector in match_sectors(haystack) if sector != "other"]
    lowered = haystack.lower()

    for sector_id, terms in BCG_CORE_SECTOR_TERMS.items():
        if sector_id in sectors:
            continue
        if any(_bcg_core_term_matches(lowered, term) for term in terms):
            sectors.append(sector_id)

    return sectors


def _bcg_core_term_matches(text: str, term: str) -> bool:
            ...
            for index, (source_key, source_url, detail_url) in enumerate(detail_targets, start=1):
                ...
                try:
                    ...
                    if not matched_sectors:
                        log.info(
                            "BCG sector 관련성 낮아 제외 | title=%s url=%s",
                            title,
                            detail_url,
                        )
                        continue
