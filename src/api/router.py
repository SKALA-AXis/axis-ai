...
# 작성자: 최종민
# 변경이력:
#   2026-04-21 최종민 — axis-ai 라우터 베이스라인 구축, 이후 수집/분석 prototype 엔드포인트(insight·mixer·global-trends·link·chat·classify)와 SSE 스트리밍·OOM/관측성·성능 개선 추가
#   2026-04-28 박지원 — 크롤러 로직 개선 및 뉴스 전처리/클러스터링 품질 개선, 카드뉴스 통합/요약/시사점 생성 및 전처리 LLM fallback 제거
#   2026-05-11 심유정 — 카드뉴스 openapi 스키마 정합·생성 플로우 개선, 브리핑 생성 API 및 사용자 전략 컨텍스트 오버레이 추가
#   2026-06-04 박진 — 통합 이슈 기반 mixer/브리핑 플로우, 투데이 인사이트 에이전트 및 사용자 챗봇 API 라우트 추가
import asyncio
import base64
import binascii
import copy
