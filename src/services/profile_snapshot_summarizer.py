...
- 출력은 반드시 간결한 JSON 객체 1개만 반환합니다. 설명 문장, markdown, 코드블록은 금지입니다.
- summary는 240자 이내, 각 문자열 필드는 120자 이내로 압축합니다.
- initiatives는 최대 5개, strategy_context는 최대 8개만 남깁니다.
- 각 배열 필드는 핵심 6개 이내로 제한하고, 원문 문단을 길게 복사하지 않습니다.
- 입력이 길어도 모든 문장을 보존하려 하지 말고 카드 대응방향 판단에 재사용 가능한
  명칭, 범위, 제약, 판단 기준만 추립니다.
