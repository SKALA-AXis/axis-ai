...
4. 그 다음 basis를 바탕으로 sentence/evidence_sentence를 작성합니다.
   ...
   무엇을 보라는지 흐린 넓은 표현은 쓰지 말고, 입력 fact에서 직접 확인된
   대상·수치·기능·적용 방식·관계 구조에서 판단 축을 좁힙니다.
...
8. suggested_action은 SK AX가 내부적으로 점검할 대상과 기준만 씁니다.
   ...
   그 anchor로 변화/차이를 설명할 수 있고 SK AX가 확인할 판단 기준을 만들 수 있으면,
   ...
   - adjacent_opportunity_probe: 직접 근거는 약하지만 고객군, 자동화, 운영 시스템,
     ...
     suggested_action을 입력 fact에서 확인되는 낮은 강도의 확인/비교/검증 문장으로 다시 낮춥니다.
   ...
   SK AX가 확인할 내부 범위, 보완 여부, 판단 기준 중 무엇을 볼지 정합니다.
   action_basis에는 반드시 대응 대상, 판단 기준, 실행 관점을 각각 명확히 적습니다.
...
13. suggested_action은 "강화해야 한다", "높여야 한다"로 끝내지 말고,
    ...
    반복하지 말고, 왜 그 사실이 SK AX의 내부 비교·점검 축으로 이어지는지 설명합니다.
    대응방향 근거는 "피어사가 했기 때문"에서 멈추지 말고,
    현재 사건이 SK AX에게 어떤 대상을 확인하게 만드는지 연결합니다.
    대응방향 sentence는 SK AX가 비교할 선택지를 보여줘야 합니다.
    선택 축은 입력 근거에 실제로 나온 대상·수치·기능·관계 구조에서 가져옵니다.
...
16. 재무/거래구조 사건은 기술/사업역량으로 건너뛰지 말고, 거래 비중·매출 구성·비교군을
    ...
    suggested_action은 SK AX가 내부적으로 확인할 관리 기준과 확인 항목을 씁니다.
...
- frontend_ready 생성 가능성은 고객명/계약/구축 범위의 유무만으로 닫지 말고
  ...
  입력 fact에서 직접 확인된 대상과 변화 범위를 내부 판단 축으로 낮춥니다.
  action이 넓게 남으면 frontend_ready로 쓰지 않습니다. 현재 사건에서 실제 anchor를 골라,
  SK AX가 구분해야 할 판단 축을 입력 사건에 맞게 새로 작성합니다.
...
- frontend_ready.suggested_action.sentence는 1문장으로,
  SK AX가 무엇을 확인하고 어떤 의사결정으로 이어가야 하는지 씁니다.
  핵심 행동과 판단 기준만 짧게 쓰고, 피어사의 기능명·제품명·세부 모듈 나열은
  sentence에 넣지 않습니다. 길어지는 설명은 evidence_sentence에 둡니다.
  sentence는 자연스러운 하나의 의사결정 문장이어야 합니다. "무엇을 무엇과
  나눠 비교해야 한다", "나눠 설명해야 한다", "나눠 점검해야 한다" 같은 기계적 표현으로
  끝내지 말고, 현재 사건에서 확인된
  수치·기능·적용처가 SK AX의 어떤 영업/제안/운영 판단으로 이어지는지 씁니다.
  내부 basis 조각이나 기사 표현을
  기계적으로 붙여 만든 명사 나열형 문장, 같은 판단 축을 두 번 반복하는 문장은 실패입니다.
  ...
  연결 근거가 강할 때만 더 실행적인 선택지를 씁니다.
  연결 근거가 약할 때는 낮은 강도의 수요 확인, 적용 가능성 확인, 우선순위 추적
  문장으로 낮춥니다.
- frontend_ready.suggested_action.sentence는 key_implication.sentence의 핵심 명사 조합을
  그대로 반복하지 않습니다. key_implication은 피어/시장 의미,
  suggested_action은 SK AX의 대응 범위와 판단 기준을 명확히 구분합니다.
  ...
  여러 피어사의 서로 다른 사실을 한 근거 문장에 묶을 때는 뭉뚱그리지 말고,
  공통 비교축이 무엇인지 먼저 좁힙니다. 공통축을 만들 수 없으면 하나의 주요 사실만
  선택해 대응방향 근거로 씁니다.
...
- frontend_ready.suggested_action.evidence_sentence는 왜 그 점검이 필요한지 현재 사건 신호와
  ...
  입력 fact에서 확인되는 실행 구조가 SK AX의 어떤 비교·점검 대상으로 이어지는지 설명합니다.
  대응방향 근거는 점검이 필요하다고 선언하는 데서 멈추지 말고,
  현재 사건에서 드러난 차이·범위·단계·구조를 직접 설명합니다.
  문장의 끝은 SK AX가 점검해야 한다는 결론을 반복하지 말고,
  현재 사건에서 확인된 제품·수치·적용 범위·운영 단계가 어떤 비교 대상을
  남기는지까지 풀어 씁니다.
  ...
  suggested_action.evidence_sentence는 부정형 한계 문장으로 끝내지 말고,
  다음에 분리·비교·검증해야 할 대상을 구체적으로 보이게 마무리합니다.
  suggested_action.sentence에는 입력 근거에 맞는 선택 축을 하나 이상 담습니다.
