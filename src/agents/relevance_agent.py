...
class SupervisorAgent:
    ...
    async def run(self, peer_ids: list[str]) -> SupervisorResult:
        ...
        issue_clusters = self._deduplicate(credible_articles)
        classified_clusters = self._classify(issue_clusters)
    ...
    def _filter_credible(self, articles: list[RawArticle]) -> list[RawArticle]:
        ...
        for article in articles:
            credibility = self._classify_credibility(article)
    ...
    def _classify_credibility(self, article: RawArticle) -> str:
        url = getattr(article, "url", "")
        source_name = getattr(article, "source_name", "")

        if hasattr(self.credibility_agent, "classify_detail"):
            detail = self.credibility_agent.classify_detail(
                url=url,
                source_name=source_name,
            )
            return detail.get("credibility", "Unverified")

        classify = self.credibility_agent.classify
        signature = inspect.signature(classify)

        if "source_name" in signature.parameters:
            return classify(url=url, source_name=source_name)

        return classify(url)

    def _deduplicate(self, articles: list[RawArticle]) -> list[dict[str, Any]]:
        return self.dedup_agent.run(articles)

    def _classify(self, issue_clusters: list[dict[str, Any]]) -> list[dict[str, Any]]:
        classified_clusters: list[dict[str, Any]] = []

        for cluster in issue_clusters:
            cluster_articles = [
                self._article_to_dict(article)
    ...
    def _make_issue_cards(self, classified_clusters: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [self.issue_card_agent.generate(cluster) for cluster in classified_clusters]

    def _make_implications(self, issue_cards: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    def _article_to_dict(self, article: RawArticle) -> dict[str, Any]:
        ...
        return {
            ...
            "source_name": getattr(article, "source_name", ""),
            "peer_id": getattr(article, "peer_id", ""),
        }